...
if (file_exists("./function-ajay.php")) {
    require_once "./function-ajay.php";
}elseif (file_exists("../function-ajay.php")) {
    require_once "../function-ajay.php";
}
...
function lastAutoBidOnCar($acm_id, $ar_id)
{
    global $conn;
...
function isOnTrial(){
    if(isset($_SESSION['ar_id'])){
        $joined_on = getValue('ambit_registration', 'joined_on', $_SESSION['ar_id'], 'ar_id');
        $trialDays = 28;
        $trialDate = (new Carbon\Carbon($joined_on))->addDays($trialDays - 1)->endOfDay();
        return $trialDate->gt(Carbon\Carbon::now());
    }
    return false;
}
