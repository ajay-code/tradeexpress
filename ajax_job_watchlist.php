...
?> 
 <section id="loop_classified_taxonomy" class="search_result_listing list" >
		...
		$con1=$con2=$con3=$con4=$con5=$con6=$con7=$con8=$con9=$con10=$con11=array();
		...
		$from="add_job as a,add_to_watch_job as b";
		$condition=array("status"=>1,'a.aj_id'=>'b.item_id','cus_id'=>$_SESSION["ar_id"]);
		$condition=array_merge($condition,$con1,$con2,$con3,$con4,$con5,$con6,$con7,$con8,$con9,$con10,$con11);
		$job_dtls_limit=getDetails(doSelect1($select,$from,$condition,$limit));
		$job_dtls_total=getDetails(doSelect1($select,$from,$condition));
                            ...
                            <div class="entry">
								<div class="sort-title">
                         			<div class="classified-title">
										<h2 class="entry-title"><a itemprop="url" href="job_details.php?id=<?php echo $v["aj_id"]; ?>" title="<?php echo $v["listing_title"]; ?>" rel="bookmark"><?php echo $v["listing_title"]; ?></a></h2>
...
?>		
	...
	if($check){
	?>
	<span id="tmplfavorite_1080" class=""><i class="fa fa-binoculars" style="font-size:1em;color:#F0B74A;"></i><a title="Add to watchlist" href="javascript:void(0);"  onclick="add2watch_general_item('<?php echo $v["aj_id"]; ?>','add_to_watch_job','watch_span_j_<?php echo $v["aj_id"]; ?>'); ajax_pagintn11(1,1);" > Remove from watchlist</a></span>
	<?php
	}else{
	?>
	<span id="tmplfavorite_1080" class=""><i class="fa fa-binoculars" style="font-size:1em;color:grey;"></i><a title="Add to watchlist" href="javascript:void(0);"  onclick="add2watch_general_item('<?php echo $v["aj_id"]; ?>','add_to_watch_job','watch_span_j_<?php echo $v["aj_id"]; ?>'); ajax_pagintn11(1,1);" > Add to watchlist</a></span>
	<?php	
	}
	}else{
	?>
