...
function getSlideDetails($query){
	global $conn;
	$dbdata=array();
...
function getDetails($query){	$dbdata=array();	$rows=mysqli_num_rows($query);	if($rows >= 1){		while($data=mysqli_fetch_assoc($query)){			array_push($dbdata, $data);		}
	}
	return $dbdata;
}
...
function getItemReviewNumber($db,$item){
	return trim(seeMoreDetails("count(*)",$db,array("status"=>1)));
}
