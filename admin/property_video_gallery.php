...
if(isset($_GET["id"])){
$property_id=$_GET["id"];
$condition=array("ap_id"=>$property_id);
}
...
function approval_property_video(apv_id){
		$.post(
		"ajax_suspend.php",
		{apv_id:apv_id,msg:"approval_property_video"},
		function(r){
			if(r==1){
				location.reload();
			}else{
...
function delete_property_video(apv_id){
		//alert("sidd");
		if(confirm("Are You Sure To delete Video ???")){
		$.post(
		"ajax_delete.php",
		{apv_id:apv_id,msg:"delete_property_video"},
		function(r){
			if(r==1){
...
<option value=".folder2">Approved Video</option>
<option value=".folder3">Suspend Video</option>
...
echo getPropertyTitle($property_id);  
}else{
	echo 'ALL PROPERTY VIDEOS';
}
?>	
...
$property_approved_video=getDetails(doSelect1("apv_id,ap_id,video,status","add_property_video",$where));


foreach($property_approved_video as $k=>$v){
?>
...
<iframe class="embed-responsive-item" src="<?php echo $v["video"]; ?>" frameborder="0" allowfullscreen></iframe>
</div>
</div>
<div class="row table-layout">
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-trash-o fs20" onclick="delete_property_video('<?php echo $v["apv_id"];  ?>');" data-toggle="tooltip" title="To Delete Click Here"  style="cursor:pointer; color: red;"></span>
</div>
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-pause fs20"  onclick="approval_property_video('<?php echo $v["apv_id"];  ?>');" data-toggle="tooltip" title="To Suspend Click Here"  style="cursor:pointer; color: #783a3a;"></span>
</div>  
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getPropertyTitle($v["ap_id"]).' Not Approved ';else echo getPropertyTitle($v["ap_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
...
$where=array_merge($where,$condition);
$property_suspend_video=getDetails(doSelect1("apv_id,ap_id,video,status","add_property_video",$where));


foreach($property_suspend_video as $k=>$v){
?>
...
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-play-circle fs20"  onclick="approval_property_video('<?php echo $v["apv_id"];  ?>');" data-toggle="tooltip" title="To Approve Click Here"  style="cursor:pointer;color: #3151c0;"></span>
</div> 
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getPropertyTitle($v["ap_id"]).' Not Approved ';else echo getPropertyTitle($v["ap_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
<!-- text-danger for red icon -->
