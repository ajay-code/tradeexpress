...
function approval_property_photo(app_id){
		$.post(
		"ajax_suspend.php",
		{app_id:app_id,msg:"approval_property_image"},
		function(r){
			if(r==1){
				location.reload();
			}else{
...
function delete_property_photo(app_id){
		//alert("sidd");
...
$property_approved_photo=getDetails(doSelect1("app_id,ap_id,image,status","add_property_photo",$where));


foreach($property_approved_photo as $k=>$v){
?>

 <div class="mix folder2">
<div class="panel p6 pbn">
<div class="of-h">
<img src="../property_image/<?php echo $v["image"];  ?>" class="img-responsive" title=""  width="200" height="200">
<div class="row table-layout">
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-trash-o fs20" onclick="delete_property_photo('<?php echo $v["app_id"];  ?>');" data-toggle="tooltip" title="To Delete Click Here"  style="cursor:pointer; color: red;"></span>
</div>
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-pause fs20"  onclick="approval_property_photo('<?php echo $v["app_id"];  ?>');" data-toggle="tooltip" title="To Suspend Click Here"  style="cursor:pointer; color: #783a3a;"></span>
</div>  
...
$where=array_merge($where,$condition);
$property_suspend_photo=getDetails(doSelect1("app_id,ap_id,image,status","add_property_photo",$where));


foreach($property_suspend_photo as $k=>$v){
?>
...
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-play-circle fs20"  onclick="approval_property_photo('<?php echo $v["app_id"];  ?>');" data-toggle="tooltip" title="To Approve Click Here"  style="cursor:pointer;color: #3151c0;"></span>
</div> 
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getPropertyTitle($v["ap_id"]).' Not Approved ';else echo getPropertyTitle($v["ap_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
<!-- text-danger for red icon -->
