...
$condition=array();
if(isset($_GET["id"])){
$general_item_id=$_GET["id"];
$condition=array("agi_id"=>$general_item_id);
}
...
function approval_item_photo(agip_id){
		$.post(
		"ajax_suspend.php",
		{agip_id:agip_id,msg:"approval_general_item_image"},
		function(r){
			if(r==1){
				location.reload();
			}else{
...
function delete_item_photo(agip_id){
		//alert("sidd");
...
<option value="">
<?php
if(isset($general_item_id)){ 
echo getGeneralItemTitle($general_item_id);  
}else{
	echo 'ALL GENERAL ITEMS PHOTOS';
}
?>	
...
$items_approved__photo=getDetails(doSelect1("agip_id,agi_id,image,status","add_general_items_photo",$where));


foreach($items_approved__photo as $k=>$v){
?>

 <div class="mix folder2">
<div class="panel p6 pbn">
<div class="of-h">
<img src="../general_items_image/<?php echo $v["image"];  ?>" class="img-responsive" title=""  width="200" height="200">
<div class="row table-layout">
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-trash-o fs20" onclick="delete_item_photo('<?php echo $v["agip_id"];  ?>');" data-toggle="tooltip" title="To Delete Click Here"  style="cursor:pointer; color: red;"></span>
</div>
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-pause fs20"  onclick="approval_item_photo('<?php echo $v["agip_id"];  ?>');" data-toggle="tooltip" title="To Suspend Click Here"  style="cursor:pointer; color: #783a3a;"></span>
</div>  
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getGeneralItemTitle($v["agi_id"]).' Not Approved ';else echo getGeneralItemTitle($v["agi_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
...
$where=array_merge($where,$condition);
$item_suspend__photo=getDetails(doSelect1("agip_id,agi_id,image,status","add_general_items_photo",$where));


foreach($item_suspend__photo as $k=>$v){
?>
...
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-play-circle fs20"  onclick="approval_item_photo('<?php echo $v["agip_id"];  ?>');" data-toggle="tooltip" title="To Approve Click Here"  style="cursor:pointer;color: #3151c0;"></span>
</div> 
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getGeneralItemTitle($v["agi_id"]).' Not Approved ';else echo getGeneralItemTitle($v["agi_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
<!-- text-danger for red icon -->
