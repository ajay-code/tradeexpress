...
function approval_car_photo(acmp_id){
		$.post(
		"ajax_suspend.php",
		{acmp_id:acmp_id,msg:"approval_car_image"},
		function(r){
			if(r==1){
				location.reload();
			}else{
...
function delete_car_photo(acmp_id){
		//alert("sidd");
		if(confirm("Are You Sure To delete Image ???")){
		$.post(
		"ajax_delete.php",
		{acmp_id:acmp_id,msg:"delete_car_image"},
		function(r){
			if(r==1){
...
<?php
if(isset($car_id)){ 
echo getCarTitle($car_id);  
}else{
	echo 'ALL ITEM PHOTOS';
}
?>	
...
$car_approved_photo=getDetails(doSelect1("acmp_id,acm_id,image,status","add_car_motorcycle_photo",$where));


foreach($car_approved_photo as $k=>$v){
?>

 <div class="mix folder2">
<div class="panel p6 pbn">
<div class="of-h">
<img src="../car_motorcycle_image/<?php echo $v["image"];  ?>" class="img-responsive" title=""  width="200" height="200">
<div class="row table-layout">
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-trash-o fs20"  onclick="delete_car_photo('<?php echo $v["acmp_id"];  ?>');" data-toggle="tooltip" title="To Delete Click Here"  style="cursor:pointer; color: red;"></span>
</div>
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-pause fs20"  onclick="approval_car_photo('<?php echo $v["acmp_id"];  ?>');" data-toggle="tooltip" title="To Suspend Click Here"  style="cursor:pointer; color: #783a3a;"></span>
</div>  
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getCarTitle($v["acm_id"]).' Not Approved ';else echo getCarTitle($v["acm_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
...
$where=array_merge($where,$condition);
$car_suspend_photo=getDetails(doSelect1("acmp_id,acm_id,image,status","add_car_motorcycle_photo",$where));


foreach($car_suspend_photo as $k=>$v){
?>
...
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-play-circle fs20"  onclick="approval_car_photo('<?php echo $v["acmp_id"];  ?>');" data-toggle="tooltip" title="To Approve Click Here"  style="cursor:pointer; color: #3151c0;"></span>
</div> 
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getCarTitle($v["acm_id"]).' Not Approved ';else echo getCarTitle($v["acm_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
<!-- text-danger for red icon -->
