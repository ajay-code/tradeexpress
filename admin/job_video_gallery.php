...
function approval_job_video(ajv_id){
		$.post(
		"ajax_suspend.php",
		{ajv_id:ajv_id,msg:"approval_job_video"},
		function(r){
			if(r==1){
				location.reload();
			}else{
...
function delete_job_video(ajv_id){
		//alert("sidd");
		if(confirm("Are You Sure To delete Video ???")){
		$.post(
		"ajax_delete.php",
		{ajv_id:ajv_id,msg:"delete_job_video"},
		function(r){
			if(r==1){
...
$job_approved_video=getDetails(doSelect1("ajv_id,aj_id,video,status","add_job_video",$where));


foreach($job_approved_video as $k=>$v){
?>
...
<span class="fa fa-trash-o fs20" onclick="delete_job_video('<?php echo $v["ajv_id"];  ?>');" data-toggle="tooltip" title="To Delete Click Here"  style="cursor:pointer; color: red;"></span>
</div>
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-pause fs20"  onclick="approval_job_video('<?php echo $v["ajv_id"];  ?>');" data-toggle="tooltip" title="To Suspend Click Here"  style="cursor:pointer; color: #783a3a;"></span>
</div>  
...
$where=array_merge($where,$condition);
$job_suspend_video=getDetails(doSelect1("ajv_id,aj_id,video,status","add_job_video",$where));


foreach($job_suspend_video as $k=>$v){
?>
...
<div class="col-xs-4 va-m pln" style="color:red;">
<span class="fa fa-play-circle fs20"  onclick="approval_job_video('<?php echo $v["ajv_id"];  ?>');" data-toggle="tooltip" title="To Approve Click Here"  style="cursor:pointer;color: #3151c0;"></span>
</div> 
<div class="col-xs-4 text-right va-m prn">
<span style="cursor:pointer;"  data-toggle="tooltip"  title="<?php if($v["status"]==0)echo getJobTitle($v["aj_id"]).' Not Approved ';else echo getJobTitle($v["aj_id"]).' is Approved';  ?> " class="fa fa-circle fs10 <?php if($v["status"]==0){echo ' text-danger ';}else { echo 'text-info';  }  ?> ml10"></span>
<!-- text-danger for red icon -->
