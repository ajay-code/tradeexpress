    ...
    if (isset($_POST['submit_auto_bid'])) {
        ...
        if ($data['auto_bid'] >= $prev_bid + 1 && $data['bid_price'] <= $data['auto_bid']) {
            if (alreadyMadeBidOnCar($_POST["acm_id"], $_SESSION['ar_id'])) {
				$last_bid = lastBidOnCar($_POST["acm_id"], $_SESSION['ar_id']);
				if($last_bid < $prev_bid){
					$status = updateBidOnCar($_POST["acm_id"], $_SESSION['ar_id'], $data['bid_price']);
				} 
                $status = updateAutoBidOnCar($_POST["acm_id"], $_SESSION['ar_id'], $data['auto_bid']);
                // $status = updateEmailMeOnCar($_POST["acm_id"], $_SESSION['ar_id'], $data['email_me']);
            } else {
                $status=doInsert($data, "ambit_general_car");
            }
        }
        // then refirect
        header("LOCATION: car_details.php?id={$_POST["acm_id"]}");
        exit();
    }
    $auto_bid_ammount =  $_POST['auto_bid_amount'];
    $acm_id = $_POST['acm_id'];
    $field = "acm_id, uploader, listing_title";
    $v = getValueDetails('add_car_motorcycle', $field, $acm_id, 'acm_id');
				...
				<form method="post">
					...
					<div class="row">
						...
						<div class=" col-12 text-center caution">
							...
							<a href="/car_details.php?id=<?=$_POST["acm_id"]?>">
