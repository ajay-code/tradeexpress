	...
	<aside id="sidebar-primary" class="sidebar large-3 small-12 columns">
		...
		<ul class="horizontal_location_nav">
			...
			<div class="">
				<h4>Category</h4>
				<li>
				<select name="category" id="category" onchange="ajax_pagintn('1');">
						<option value="">--Select--</option>
					<?php
						$categories=getDetails(doSelect("gic_id,category","general_item_category",array("parent_id"=>0)))
					?>
					<?php foreach($categories as $index => $category): ?>
						<option value="<?= $category['gic_id'] ?>"><?= $category['category']?></option>
					<?php endforeach; ?>
				</select>
				</li>
				...
				<input name="min_price" id="min_price" value="" onkeyup="ajax_pagintn('1');" type="text" class="textfield ">	
				<label class=r_lbl>Maximum Price</label>	
				<input name="max_price" id="max_price" value="" onkeyup="ajax_pagintn('1');" type="text" class="textfield ">	
			</div>
			 <!-- division for range slider -->
						<h4>Brand Item</h4>
						 
			 <li>
				  <select name="brand_item" id="brand_item" filterdisplayname="State" onchange="ajax_pagintn('1');">
