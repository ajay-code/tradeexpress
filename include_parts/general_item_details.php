...
<?php
    ...
    if (getNumberOfBids($v['agi_id'])>0):?>
							<label>Current Bid: </label> <span class='text-right frontend_zip_code' ><?php echo '$'.getCurrentBid($v["agi_id"]);
    $next_bid_price=getCurrentBid($v["agi_id"]); ?></span></p>
						<?php else:?>
    ...
    $next_bid_price=$v["start_price"]; ?></span></p>
        ...
        //var_dump(getCurrentBid($v["agi_id"]).'/'.$v["reserve_price"]);
        if ($v["reserve_price"]<=getCurrentBid($v["agi_id"])) {
            echo 'Reserve met!';
        } else {
    ...
    // var_dump($v["sold_status"],$v["status"], $flag, $show);?>
                    ...
                    if ($flag==1 && $show==1) {
						...
						<div class="item-design item-height">
								<label class="label-margins"><input type="checkbox" name="auto-bid"> Auto-bid</label>
