...
/* function ajax_pagintn1111(page,limit){
	var query_string='<?php echo $_SERVER["QUERY_STRING"]; ?>';
	$.post(
	'ajax_car_pagination.php?page='+page+'&'+query_string+'&limit='+limit,
	{},
	function(r){
		$("#ajax_page").html(r);
	}
...
function ajax_pagintn111(page,limit=10){
	...
	var data='min_price='+min_price+'&max_price='+max_price+'&brand_item='+brand_item+'&page='+page+'&limit='+limit+'&classified_sortby='+classified_sortby;
	$.post(
	'ajax_car_pagination.php?'+query_string+'&'+data,
	{},
	function(r){
		$("#ajax_page").html(r);
	}
...
<span class="sep">&raquo;</span> <span class="trail-end">Car, Motorbikes & Boats</span></div>
</div>
<div id="update_ajax">
...
<div id="sub_listing_categories">
	...
	<li class="cat-item cat-item-18">Category
			...
			foreach($category as $k=>$v){
				$select="";
				if(isset($_GET["category"]) && $_GET["category"]!="" && $_GET["category"]==$v["cc_id"]){
				$select='style="color:#899516;"';	
				}
			?>
				<li class="cat-item cat-item-47"><a href="car_listing.php?category=<?php echo $v["cc_id"]; ?>" <?php echo $select; ?> ><?php echo $v["category"]; ?></a></li>
			<?php
			}
			?>
			</ul>
	...
	<li class="cat-item cat-item-18">Sub-Category
			<ul class='children'>
				<?php
			$category=getDetails(doSelect("cc_id,category","car_category",array("parent_id"=>$_GET["category"])));
			foreach($category as $k=>$v){
				$select="";
				if(isset($_GET["sub_category"]) && $_GET["sub_category"]!="" && $_GET["sub_category"]==$v["cc_id"]){
				$select='style="color:#899516;"';	
				}
			?>
				<li class="cat-item cat-item-44"><a href="car_listing.php?category=<?php echo $_GET["category"]; ?>&sub_category=<?php echo $v["cc_id"]; ?>" <?php echo $select; ?> ><?php echo $v["category"]; ?></a></li>
			<?php
			}
			?>
			</ul>
	...
	<li class="cat-item cat-item-18">Sub-Sub-Category
			<ul class='children'>
			<?php
			$category=getDetails(doSelect("cc_id,category","car_category",array("parent_id"=>$_GET["sub_category"])));
			foreach($category as $k=>$v){
				$select="";
				if(isset($_GET["sub_sub_category"]) && $_GET["sub_sub_category"]!="" && $_GET["sub_sub_category"]==$v["cc_id"]){
				$select='style="color:#899516;"';	
				}
			?>
				<li class="cat-item cat-item-46"><a href="car_listing.php?category=<?php echo $_GET["category"]; ?>&sub_category=<?php echo $_GET["sub_category"]; ?>&sub_sub_category=<?php echo $v["cc_id"]; ?>" <?php echo $select; ?> ><?php echo $v["category"]; ?></a></li>
			<?php
			}
			?>
			</ul>
	...
	<div class="sort_options">
			...
			<form action="classifieds/classified/?" method="get" id="classified_sortby_frm" name="classified_sortby_frm">
               <select name="classified_sortby" id="classified_sortby" onChange="ajax_pagintn111('1');" class="tev_options_sel">
...
<!--taxonomy  sidebar -->
<?php
include("include_parts/car_filter_sidebar.php");
?>
</div>
